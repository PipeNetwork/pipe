...
Authenticate again using `auth login`; it opens a browser on a local desktop and automatically prints a URL/code on a remote or headless terminal. `--no-browser` remains available when an explicit printed URL is preferred. Approval is explicit in the browser. Wallet login uses generalized platform authentication; the preserved storage-only endpoints require the explicit combination `auth login --wallet FILE --legacy-wallet` and compatible server support. There is no automatic legacy fallback. Request additional permissions with `--scope`; session listing/revocation requires `credentials.read`/`credentials.write` respectively. For automation, use an activated scoped credential through `PIPE_CLI_TOKEN`, not a website token or product key.

New secrets use the OS keyring when it is available. On Linux or another headless terminal without a usable keyring, the CLI automatically creates a random private machine key and encrypts its local store without asking the user for another password. `PIPE_CLI_SECRET_PASSWORD` is only needed to unlock an older passphrase-protected store or when automation supplies an explicit secret. A legacy plaintext fallback remains readable; its next secret update writes encrypted storage and creates a private backup first. Keep the state directory and machine key together. Keyring failures never authorize new plaintext storage; secret export requires `--show-secret`.
