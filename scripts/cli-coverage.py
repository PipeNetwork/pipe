...
storage_workspace_commands = {'storageBucketCredentialsList':'pipe storage keys list','storageBucketCredentialCreate':'pipe storage keys create','storageBucketCredentialRevoke':'pipe storage keys revoke','storageBucketsList': 'pipe storage buckets list', 'storageBucketsCreate': 'pipe storage buckets create', 'storageBucketGet': 'pipe storage buckets get', 'storageBucketAudit': 'pipe storage audit', 'storageObjectsSearch': 'pipe storage search', 'storageCorsGet': 'pipe storage cors get', 'storageCorsUpdate': 'pipe storage cors set', 'storageWebhooksList': 'pipe storage webhooks list', 'storageWebhookCreate': 'pipe storage webhooks create', 'storageWebhookRevoke': 'pipe storage webhooks revoke', 'storageWebhookDeliveries': 'pipe storage webhooks deliveries', 'storageWebhookRetry': 'pipe storage webhooks retry', 'storageLifecycleGet': 'pipe storage lifecycle get', 'storageLifecyclePreview': 'pipe storage lifecycle preview', 'storageLifecycleUpdate': 'pipe storage lifecycle enable / disable', 'platform.organization_audit': 'pipe org audit'}
compute_commands = {
...
for service,file in files.items():
 ...
 for path,item in items:
  for method,op in item.items():
   ...
   elif oid in storage_workspace_commands:
    status='implemented';disposition='dedicated';command=storage_workspace_commands[oid];reason='Account-bound encrypted original requests, exact decimal sizes, bounded cursor pagination, revision/preview checks and explicit secret output. Qualification and deployment are tracked separately.'
   ...
   rows.append({'service':service,'operation_id':oid,'method':method.upper(),'path':path,'disposition':disposition,'implementation_status':status,'command':command,'reason':reason,'credentials':op.get('security',[]),'feature_conditions':op.get('x-pipe-availability',{}),'retry_policy':'read-only' if method in ['get','head'] else 'resume-exact-original-request' if oid in storage_workspace_commands else 'managed-billing-journal-recovery' if oid in billing_workflows else 'observe-current-state-never-replay' if oid in customer_commands or oid in hosting_commands and oid not in hosting_replay else 'read-current-setting-never-replay' if oid=='updateDurableLimit' else 'resume-exact-idempotency-only' if oid in compute_commands or oid in kv_commands or oid in durable_commands or oid in hosting_replay else 'managed-workflow-only','verification_fixture':('storage_workspace_tests; storage_automation_tests; s3::delete_pricing_tests; CLI storage_workspace tests' if oid in storage_workspace_commands else 'pipenetwork::billing::tests::platform_cli_billing_hosted_replay_and_sandbox_settlement; CLI billing_workflows tests' if oid in billing_workflows else 'advanced::tests; lattice-router tests::cli_diagnostics::executable_public_diagnostics_preserve_legacy_credit_and_never_forward_credentials; control public contracts' if oid in public_diagnostics else 'compute::tests::platform_cli_executable_wallet_and_vm_lifecycle; CLI compute_tests; pipe-transports openssh fixture' if oid in compute_commands else 'kv::tests::platform_cli_executable_kv_management_recovery; scripts/test-kv-workflow.py; pipe-transports tests/kv.rs' if oid in kv_commands else 'durable::tests::platform_cli_real_storage_workflow; CLI durable_tests; scripts/test-customer-durable.py' if oid in durable_commands else 'hosting::integration_tests::platform_cli_real_host_workflow; scripts/test-cli-hosting.py; CLI hosting_tests' if oid in hosting_commands else 'pipenetwork::cli_tests::platform_cli_canonical_accounts_organizations_and_billing; CLI customer_tests' if oid in customer_commands else 'cli_auth::tests; tests/cli_contract.rs' if oid.startswith('platformCli') else 'baseline cargo test' if status=='implemented' else None), 'advanced_access':(oid in storage_workspace_commands or oid == 'getStoragePricing' or oid in public_diagnostics or oid in compute_commands or oid in kv_commands or oid in hosting_commands or oid in customer_commands and oid!='platform.referral_summary' or (oid in durable_commands and oid!='getDurableOperation')) and method=='get', 'required_cli_scopes':op.get('x-pipe-cli-scopes',op.get('x-pipe-required-scopes',[])), 'conditional_cli_scopes':op.get('x-pipe-cli-conditional-scopes',{}), 'allowed_cli_credential_kinds':op.get('x-pipe-cli-credential-kinds',[])})
# This additive read bootstrap creates a credential; its permission is read-only,
# but the POST itself must never be classified as a retryable read.
for row in rows:
 if row['operation_id']=='cliCreateS3StorageSession':
  row.update(command='pipe s3 setup / ls / head / object list (read onboarding)',
             reason='Short-lived delegated read/list storage credential; write setup uses cliCreateS3Credential after obtaining storage.write and credentials.write.',
             required_cli_scopes=['storage.read'],retry_policy='never-replay',
             verification_fixture='tests/cli_contract.rs; openapi_contract tests')
